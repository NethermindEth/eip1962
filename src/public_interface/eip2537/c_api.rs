...
{            
    ...
    let err_out_i8: &mut [i8] = unsafe { std::slice::from_raw_parts_mut(err as *mut i8, EIP2537_PREALLOCATE_FOR_ERROR_BYTES) };
    ...
    let input_i8: & [i8] = unsafe { std::slice::from_raw_parts(i as *mut i8, i_len as usize) };
    let input: &[u8] = unsafe { std::mem::transmute(input_i8) };

    let raw_out_i8: &mut [i8] = unsafe { std::slice::from_raw_parts_mut(o as *mut i8, EIP2537_PREALLOCATE_FOR_RESULT_BYTES) };
